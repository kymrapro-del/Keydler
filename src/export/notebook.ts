...
export function buildFullExport(tasks: readonly TaskState[]): string {
  if (tasks.length === 0) return '# No nightorder on this device\n'
  return [
    `# ${tasks.length} nightorder${tasks.length > 1 ? 's' : ''}`,
    '',
    ...tasks.map(
      (t) =>
...
export function exportFilename(task: TaskState | null): string {
  if (!task) return 'nightorders.md'
  const base = task.title
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
    .slice(0, 48)
  return `nightorder-${base || task.id}-v${task.version}.md`
}
