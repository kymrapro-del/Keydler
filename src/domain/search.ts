...
export type MatchKind = 'rule' | 'rejection' | 'step' | 'decision' | 'evidence' | 'history'
...
export function searchTask(task: TaskState, query: string): Match[] {
  ...
  for (const r of task.rejected) {
    if (matches(r.approach, q) || matches(r.reason, q)) {
      found.push({
        kind: 'rejection',
  ...
  for (const s of task.steps) {
    const inEvidence = s.evidence !== null && matches(s.evidence.content, q)
    if (matches(s.action, q) || matches(s.result, q) || inEvidence) {
      found.push({
        kind: inEvidence && !matches(s.action, q) && !matches(s.result, q) ? 'evidence' : 'step',
  ...
  for (const d of task.decisions) {
    if (matches(d.choice, q) || matches(d.rationale, q)) {
