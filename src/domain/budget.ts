/**
 * Chrome recommends 1.5 k characters per tool output, with no hard limit: so we
 * fill up to the budget and not up to a count, twelve 240-character excerpts
 * making 6296. Two exemptions: `read_task_detail`, which capped at 1.5 k
 * returned only one or two entries per page as soon as evidence was attached,
 * and `resume_task`, which has its own budget in tokens, that is 1600
 * characters, 1528 rendered in practice.
 ...
 * number of entries finally kept, so on the result of the loop that picks them.
 * Reserving generously lifts the circularity without complicating the count.
 */
