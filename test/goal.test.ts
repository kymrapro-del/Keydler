...
const GOAL = 'Every session expires in under 30 minutes, with no change to the schema.'
...
describe('what the agent reads of it', () => {
  it('places it with the next action, not at the bottom', () => {
    const rendered = renderTaskState(setGoal(task, GOAL))
  ...
  it('stays readable on a closed task, next to the summary', () => {
    const closed = completeTask(
      setGoal(task, GOAL),
      { summary: 'S', basedOnVersion: null },
      'human',
    )
...
describe('closing a task that had a goal', () => {
  ...
  it('reminds the agent of the goal, so its summary answers it', async () => {
    await store.openPreparedTask(setGoal(buildCoreTask(), GOAL))
...
describe('from the page', () => {
  ...
  it('invites you to say what "done" means when nothing is written', () => {
    const button = root.querySelector('#edit-goal')!
    expect(button).not.toBeNull()
    expect(button.textContent).toMatch(/done/i)
  })
  ...
  it('writes it, and shows it under the next action', async () => {
    ...
    await waitUntil(() => store.currentTask()?.goal === GOAL, 'the goal to be written', 3000)
    __renderNow()

    expect(root.querySelector('.hero')!.textContent).toContain(GOAL)
  })
})
...
describe('for an agent without WebMCP', () => {
  ...
  it('copies the log as text, just as the agent would have received it', async () => {
    ...
    const expected = renderTaskState(store.currentTask()!, {
  ...
  it('carries no credential value', async () => {
    ...
    for (const word of ['ciphertext', 'passphrase', 'sealed:']) {
      expect(copied, word).not.toContain(word)
    }
  })
