...
describe('recording a step by hand', () => {
  ...
  /**
   * Pasted evidence leaves with every export and every link, and a command
   * output readily carries a token. The field says so where the paste happens,
   * not in a help page.
   */
  it('says, at the moment of pasting, where the evidence will travel', () => {
    const field = root.querySelector('#step-evidence')!
    const note = field.parentElement!.textContent!.replace(/\s+/g, ' ')
  ...
  it('no longer labels a diff as command output in what the agent reads', async () => {
    ...
    // read_task_detail announced "command_output" for a diff, misreporting the
    // kind of evidence held.
