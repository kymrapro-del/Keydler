...
describe('search on screen', () => {
  ...
  it('counts the results of each section, not just the total', async () => {
    ...
    // A subheading without a count forces a manual count of the rows to know
    // whether the answer is here or elsewhere.
...
describe('filtering results by kind', () => {
  ...
  it('carries the exact count of each kind, and their sum', () => {
    ...
    for (const m of found) expected.set(m.kind, (expected.get(m.kind) ?? 0) + 1)

    const count = (button: HTMLButtonElement) =>
      Number(/\((\d+)\)\s*$/.exec(button.textContent!.trim())![1])

    for (const button of root.querySelectorAll<HTMLButtonElement>('[data-filter]')) {
      const nature = button.dataset.filter!
      expect(count(button), nature).toBe(
        nature === 'all' ? found.length : (expected.get(nature) ?? 0),
      )
    }
  ...
  it('orders the kinds the way the log presents them', () => {
    ...
    const ordre = [...root.querySelectorAll<HTMLButtonElement>('[data-filter]')]
      ...
      .filter((k) => k !== 'all')
    const expected = [...new Set(searchTask(store.currentTask()!, 'token').map((m) => m.kind))]
    expect(ordre).toEqual(expected)
  })

  it('narrows down to the rows of the chosen kind', async () => {
    const before = rows().length
    const button = [...root.querySelectorAll<HTMLButtonElement>('[data-filter]')].find(
