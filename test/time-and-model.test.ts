...
describe('carrying rules over from a log', () => {
  it('copies the rules in force, and nothing else', () => {
    ...
    const target = copyRulesInto(createTask({ title: 'New task' }), source)

    const expected = activeConstraints(source).map((c) => c.rule)
    expect(activeConstraints(target).map((c) => c.rule)).toEqual(expected)

    // Not the work, not the rejections, not the other one's write log.
    expect(target.steps).toHaveLength(0)
    expect(target.rejected).toHaveLength(0)
    expect(target.decisions).toHaveLength(0)
    expect(target.questions).toHaveLength(0)
  })

  it('makes them binding at once, and attributed to the human', () => {
    const target = copyRulesInto(createTask({ title: 'New task' }), buildCoreTask())
    for (const rule of target.constraints) {
  ...
  it('writes nothing when there is no rule to carry over', () => {
    const emptyTask = createTask({ title: 'No rules here' })
    const target = createTask({ title: 'New task' })
    expect(copyRulesInto(target, emptyTask)).toBe(target)
  })
  ...
  it('does not copy a lifted rule', () => {
    ...
    const target = copyRulesInto(createTask({ title: 'New' }), withLift)
    expect(target.constraints.some((c) => c.rule === lifted.rule)).toBe(false)
  })
})
