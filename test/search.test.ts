...
describe('search inside one task', () => {
  ...
  it('ignores case and accents', () => {
    expect(matches('Never change the schema', 'SCHEMA')).toBe(true)
  ...
  it('folds accents both ways, whichever side carries them', () => {
    expect(matches('Beyonc\u00e9', 'beyonce')).toBe(true)
    expect(matches('beyonce', 'Beyonc\u00e9')).toBe(true)
    expect(matches('BEYONC\u00c9', 'beyonce')).toBe(true)
    expect(matches('na\u00efve facade', 'naive facade')).toBe(true)
    expect(matches('Z\u00fcrich office', 'zurich')).toBe(true)

    // And it does not draw together words that the accent alone does not
    // separate.
    expect(matches('Beyonc\u00e9', 'beyond')).toBe(false)
  })

  it('folds an already decomposed accent that NFD leaves as it is', () => {
    // An accented letter written with U+0301 keeps the same search behavior, so
  ...
  it('finds a rejection by its REASON, not only by its name', () => {
    // The case that matters: looking for "why", not "what".
  ...
  it('finds a step by the content of its evidence, and says so', () => {
    ...
    // action and not in the result. The only case that proves the evidence
    // itself is searched.
...
describe('on screen', () => {
  ...
  it('replaces the dashboard with the results, and renders them', async () => {
    ...
    // During a search the sections give way: results, not a dashboard to scroll
    // past.
  ...
  it('finds another task and opens it from the results', async () => {
    ...
    await waitFor(() => store.currentTask()?.id === demoId, 'return to the demo task')
    await waitFor(() => root.querySelector('[data-open]') !== null, 'list of other tasks')
...
describe('archiving on screen', () => {
  ...
  it('archives the open task, and tells the agent', async () => {
    await store.createAndOpenTask('Current work', 'Keep going')
    await waitFor(() => root.querySelector('#archive-current') !== null, 'button')
