...
describe('poisoning by an agent', () => {
  it('holds nobody to an approach an agent condemned on its own authority', async () => {
    const task = await store.createAndOpenTask('Choose a mechanism', 'Decide')

    const written = await call(
      rejectApproachTool,
      writeArgs(task, {
        approach: 'Cookie HttpOnly',
        reason: 'assumed incompatible, not measured',
      }),
    )
  ...
  it('makes it readable, under a heading that says it binds nothing', async () => {
    ...
    await call(
      rejectApproachTool,
      writeArgs(task, { approach: 'Cookie HttpOnly', reason: 'assumed incompatible' }),
    )
  ...
  it('becomes binding as soon as a human endorses it, and says so', async () => {
    ...
    await call(
      rejectApproachTool,
      writeArgs(task, { approach: 'Cookie HttpOnly', reason: 'measured, sessions lost' }),
    )
  ...
  it('lets the human set a proposal aside without erasing it', async () => {
    ...
    await call(
      rejectApproachTool,
      writeArgs(task, { approach: 'Cookie HttpOnly', reason: 'assumed' }),
    )
  ...
  it('holds for constraints too, which forbid just as much', async () => {
    ...
    await call(addConstraintTool, writeArgs(task, { rule: 'Never touch the router' }))
    ...
    expect(rendered).toContain('constraint: Never touch the router')
  })
  ...
  it('refuses to endorse twice rather than move the version forward for nothing', async () => {
    let task = await store.createAndOpenTask('Task', 'Continue')
  ...
  it('requires the content to have been shown before it is approved', async () => {
    ...
    await call(
      ...
      writeArgs(task, {
        action: 'a',
        result: 'b',
        evidence: { kind: 'diff', content: '+ added line' },
      }),
    )
    task = currentTask()
    ...
    const approved = verifyEvidence(task, step.id, '+ added line')
